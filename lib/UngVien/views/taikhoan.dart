...
class _TaiKhoanState extends State<TaiKhoan> {
  ...
  Future<void> _loadUserData() async {
    ...
    final String cachedName = prefs.getString('userName') ?? "Người dùng";
    final String cachedEmail = prefs.getString('userEmail') ?? "";
    final String cachedAvatar = prefs.getString('avatarPath') ?? "";
    ...
    if (mounted) {
      setState(() {
        ...
        selectedAvatar = cachedAvatar.isNotEmpty ? File(cachedAvatar) : null;
        _isLoading = false;
      });
    }
    ...
    try {
      final response = await http.get(
        Uri.parse("http://localhost:5249/api/TaiKhoan/$userId"),
      );

      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);

        final String apiName = data['hoTen'] ?? data['HoTen'] ?? cachedName;
        final String apiEmail = data['email'] ?? data['Email'] ?? cachedEmail;
        final String apiCode =
            (data['idTaikhoan'] ?? data['IdTaikhoan'] ?? userId).toString();
  ...
  Future<void> _updateProfile() async {
    ...
    try {
      final response = await http.put(
        Uri.parse(
          "http://localhost:5249/api/UngVien/update-by-taikhoan/$userId",
        ),
